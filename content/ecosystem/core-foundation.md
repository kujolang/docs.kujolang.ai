...
Kujo `v1.1.0` is published with platform binaries and checksums. Its Agent
Development Platform composes explicit Kennel, Agents SDK, AI SDK, MCP, RAG,
Dispatch, Eval, Workcell, Watchdog, RunLedger, and Relay boundaries without
folding those tools into opaque hosted state. Kennel and Workcell keep local and
deployment boundaries explicit. Source is an MVP platform whose production
storage and worker paths require deployment-specific validation.
