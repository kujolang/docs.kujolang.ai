...
The original proposal needs five material corrections:
...
A Link-first reference adapter is sound **only for an explicitly supported machine purchase route**, initially Link SPT plus a registered MPP Stripe merchant route. The current CLI has additional LPT, transaction-status and test-mode support beyond older search summaries. Those details remain adapter-local. A Link-only credential retrieval wrapper would not deserve a new primitive.
